		...
		<tbody>
			...
			foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
				...
				if($_product->is_type('composite')){
					$i = 0;
					$components = $_product->get_components();
					$j = 0;
				...
				$cat = strip_tags($_product->get_categories());
				if($cat == "Electricien kosten"){
					$nComposite = 4;
				}
				$totalComposite = $nComponents + $nComposite;
				if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
					...
					<tr class="woocommerce-cart-form__cart-item <?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>">
						...
						<td class="product-remove">
							<?php
								if($compositeStart && $i < $totalComposite){
									$_product->set_sold_individually( 'yes');
									$cartKeys[] = $cart_item_key;
									if($i==0){
										$link ='<a id="compositeProduct" href="%s" class="remove" aria-label="%s" data-product_id="%s" data-product_sku="%s">&times;</a>';
									} else{
										$link ='<a style="display: none;" href="%s" class="remove" aria-label="%s" data-product_id="%s" data-product_sku="%s">&times;</a>';
									...
									echo apply_filters( // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
										...
										sprintf(
											$link,
											esc_url( WC()->cart->get_cart_url()."?removeComposite=true" ),
						...
						<?php
						if($compositeStart && $i <= $nComponents && $i> 0){
						?>
						...
						<?php
							...
							<td class="product-price" data-title="<?php esc_attr_e( 'Price', 'woocommerce' ); ?>">
							<span style="font-weight: bold;" class="woocommerce-Price-amount amount"><span class="woocommerce-Price-currencySymbol">€ </span><?php echo $_product->get_price(); ?></span>
							<small class="tax_label">(Excl. BTW)</small>
							</td>
							<?php
						}
						?>
...
<script>
	var composite = document.getElementById('compositeProduct');

	composite.addEventListener("click", removeItems)

	function removeItems(){
	
	<?php    	
	 if (isset($_GET['removeComposite'])){
	...
	?>
	window.location.href = "<?php echo esc_url( WC()->cart->get_cart_url()."?removeComposite=true" ); ?>";
	}
