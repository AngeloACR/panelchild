...
<div id="product-<?php the_ID(); ?>" <?php post_class(); ?>>
	...
	<div class="custom-product-page">
		...
		if ( get_theme_mod( 'product_custom_layout' ) ) :
			...
			$typeStep = 'step';
			if($product->is_type( $typeSimple )){
				echo do_shortcode( '[block id="nl-single-product"]');
			} elseif($product->is_type($typeComposite)){
				echo do_shortcode( '[block id="nl-composite-product-packages"]');
			} elseif($product->is_type($typeStep)) {
				echo do_shortcode( '[block id="nl-step-by-step-product"]' );
			}
			?>
